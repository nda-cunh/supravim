...
# cree une region Function Prototype sans le ';' a la fin



syntax region cFunctionDecl start="\<\h\w*\>\s\+\%(\h\w*::\)\?\h\w*\s*(" end=")" contains=cType,cCustomType,cPointer,cParameter,cOperator keepend
highlight link cFunctionDecl Normal

# syntax match cParameter "\<\h\w*\>\ze\s*[,)]" contained
# highlight link cParameter Identifier

syntax match cCustomType "\<t_\w*\>" contained
highlight link cCustomType Type



syntax match cFunction display "\<\h\w*\>[=+*/%&|^!<>\[\]]*\ze\s*("
syntax match cConstant  display "\<\u[A-Z0-9_]*\>"
...
highlight link cMember Identifier
...
elseif ft ==# "c" && !exists("c_no_c11") && !exists("c_no_cformat")
# ISO C99
...
if (ft ==# "c" && !exists("c_no_c11")) || (in_cpp_family && !exists("cpp_no_cpp11"))
# ISO C11 or ISO C++ 11
  if exists("c_no_cformat")
...
highlight link cDoxygenTag Special
highlight link cDoxygenParam Identifier
...
if !exists("c_no_c99") && !in_cpp_family
...
if exists("c_minlines")
  b:c_minlines = c_minlines
else
  if !exists("c_no_if0")
    b:c_minlines = 50	# #if 0 constructs can be long
  else
